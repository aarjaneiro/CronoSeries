...
namespace CronoSeries.TimeSeries.Transforms
{
    ...
    public abstract class TimeSeriesTransformation : IConnectable
    {
        ...
        [NonSerialized] protected List<Data.TimeSeries> outputs; // should be filled in by Recompute()
        ...
        public virtual bool SetInput(int socket, object item, StringBuilder failMsg)
        {
            ...
            var ts = item as Data.TimeSeries;
        ...
        protected int GetTotalInputDimension()
        {
            ...
            for (var i = 0; i < NumInputs(); ++i)
                ...
                {
                    var ts = socketedInputs[i] as Data.TimeSeries;
        ...
        protected List<Data.TimeSeries> GetInputBundle()
        {
            var bundle = new List<Data.TimeSeries>();
            for (var i = 0; i < NumInputs(); ++i)
                if (socketedInputs.ContainsKey(i))
                {
                    var ts = socketedInputs[i] as Data.TimeSeries;
